...
#REMARK: You may be wondering: what happens if the series we pass to the DataFrame constructor have different indexes?
# Pandas will try to combine these indexes, if possible. When that happens, the resulting series will be different
# than the original ones. Here is an example:
...
# Thus, to check if a DataFrame df is sorted in increasing order, use df.index.is_monotonic_increasing.
...
# Create a series with an unsorted index
ser_sort_inplace = pd.Series(data=[1, 2, 3], index=['a', 'c', 'b'])
